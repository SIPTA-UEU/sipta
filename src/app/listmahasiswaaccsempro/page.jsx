...
export default function ListMahasiswaAccSempro() {
  ...
  const fetchMahasiswaData = async () => {
    try {
      ...
      const mahasiswaData = mahasiswaSnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
  ...
  **MAHASISWA SEMPRO**
  ...
  `;
      ...
      const message = `Yth, Dosen Penguji 1 ${penguji1Name} dan Dosen Penguji 2 ${penguji2Name}\n\n` +
                      `Dengan hormat, berikut adalah informasi terkait mahasiswa yang akan mengikuti Seminar Proposal:\n\n` +
    ...
    <div>
      ...
      <ul className={styles.list}>
        {mahasiswaList
          .filter(
            (mahasiswa) =>
              mahasiswa.status === "Semua dokumen sesuai dan lengkap" ||
              mahasiswa.status === "Data Dikirim Ke Penguji"
          )
          .map((mahasiswa) => (
            <li key={mahasiswa.id} className={styles.listItem}>
              ...
              <p>
                <strong>Cabang Kampus:</strong> {mahasiswa.cabangKampus}
              </p>
              <p>
                <strong>Nomor WhatsApp:</strong> {mahasiswa.noWhatsapp}
              </p>
              <p>
                <strong>Judul Seminar Proposal:</strong> {mahasiswa.judul}
              </p>
              <p>
                <strong>Status:</strong>{" "}
                {mahasiswa.status || "Belum diverifikasi"}
              </p>

              {mahasiswa.penguji1 && mahasiswa.penguji2 ? (
                <div>
                  <p>
                    <strong>Dosen Penguji 1:</strong> {mahasiswa.penguji1}
                  </p>
                  <p>
                    <strong>Dosen Penguji 2:</strong> {mahasiswa.penguji2}
                  </p>
                </div>
              ) : (
                <div className={styles.buttonContainer}>
                  ...
                  >
                    ...
                    {pengujiList.map((penguji) => (
                      <option key={penguji.id} value={penguji.id}>
                        {penguji.nama}
                      </option>
                    ))}
                  </select>
                  ...
                  <button
                    ...
                    disabled={
                      !selectedPenguji[mahasiswa.id]?.penguji1 ||
                      !selectedPenguji[mahasiswa.id]?.penguji2
                    }
                  >
                    Send
                  </button>
              ...
              <p>
                File Pengajuan:{" "}
                <a
                  href={mahasiswa.pengajuanSidangUrl}
                  target="_blank"
